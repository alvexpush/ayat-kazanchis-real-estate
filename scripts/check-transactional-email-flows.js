...
const environment={...process.env,PORT:String(port),NODE_ENV:"development",DATABASE_PATH:databasePath,EMAIL_PROVIDER:"development",RESEND_API_KEY:"",DEV_AUTH_BYPASS:"true",OTP_SECRET:"email-flow-test-secret",ADMIN_EMAIL:"email-admin@example.com",ADMIN_PASSWORD:"EmailAdmin123!"};
...
(async()=>{
  try{
    ...
    const authenticated=await post("/api/login",{email,loginCode:newCode}),challenge=await authenticated.json();if(authenticated.status!==200)throw new Error("The new login code was not accepted.");const verifiedLogin=await post("/api/auth/verify-login",{email,code:challenge.developmentCode}),verifiedData=await verifiedLogin.json();if(verifiedLogin.status!==200)throw new Error(`Verified login failed: ${JSON.stringify(verifiedData)}`);const activeCookie=String(verifiedLogin.headers.get("set-cookie")||"").split(";")[0];
    ...
    const kinds=db.prepare("SELECT kind,status FROM email_deliveries ORDER BY id").all(),requiredKinds=["welcome","login_code_changed","deposit_pending","withdrawal_pending","notification_account"];
